...
class SnapshotManager(ActionManager):
    ...
    async def rollback(
        ...
        payload = {'start': 1 if start else 0}
        async with self.client.request(path, method='POST', json=payload) as resp:
            resp.raise_for_status()
            data = await resp.json()
            return data.get('data')  # Returns UPID
    ...
    async def rollback_to_latest(
        ...
        return await self.rollback(server, latest['name'], start=start)
    ...
    async def rollback_to_latest_many(
        ...
        return await self.run_bulk(
            servers,
            partial(self.rollback_to_latest, start=start),
        )
